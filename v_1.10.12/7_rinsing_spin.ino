  ...
  if (fl_0 != 1) { // Влючен отжим
    ...
    for (int i = 0; i < DEFAULT_SPIN_COUNT; i = i + 1) {
      ...
      for (int i = MOTOR_SPEED_STRAIGHTEN_MAX1; i <= MOTOR_SPEED_STRAIGHTEN_UNBALANCE; i = i + 1) {
        ...
        if (pwmSpeed > i) {
          if (pwmSpeed > MOTOR_SPEED_STRAIGHTEN_UNBALANCE * 2) {
            pwmSet = pwmSpeed;
            pwmSetMinMax();
            break;
          } else {
      ...
      if (fl_Vibro) {
        ...
        // расправление белье, цикл из 2 раз со случайно выбираемой скоростью между MOTOR_SPEED_STRAIGHTEN_MIN2  и MOTOR_SPEED_STRAIGHTEN_MAX1
        for (int j = 1; j <= 2; j = j + 1) {
          MOTOR_direction = !MOTOR_direction;
          digitalWrite(PIN_MOTOR_OUT, LOW); //Принудительно отключим мотор
          digitalWrite(PIN_MOTOR_LR, MOTOR_direction); // Установим направление вращения
          delay2(1000); // Пауза, что бы реле успело сработать
          pwmSet = random(MOTOR_SPEED_STRAIGHTEN_MIN2, MOTOR_SPEED_STRAIGHTEN_MAX1);
          pwmSetMinMax();
          MotorStart();
      ...
      if (!fl_Vibro) {
        ...
        addLog("проверка на вибрацию, на скорости MOTOR_SPEED_STRAIGHTEN_MAX4_0");
        pwmSet = MOTOR_SPEED_STRAIGHTEN_MAX4_0;
        pwmSetMinMax();
        delay4(10000); // Ждем и измеряем вибрацию
        if (fl_Vibro) {
          fl_Vibro = false;
          MotorStop();
          MotorWaitStop();
...
void rinsing_spin() {
  ...
  if (fl_PoloskanirPlus == 0) {
    ...
    water_pour(mode, WATER_LEVEL2);
    water_heating (PROGRAM_RINSING_WARM_TEMP, 10);
